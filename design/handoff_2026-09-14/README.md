...
> **Coding agent: start with `UPDATE_BRIEF.md`.** It is the current, authoritative spec and supersedes this README wherever they disagree (header, search, tab bar, value logic, origin line). This README is the original full design reference — read it for the parts `UPDATE_BRIEF.md` doesn't restate (row layout, coffee page, Insights). The visual reference is section `2a` of `MyCoffee Redesign.dc.html`; `1a` is a rejected alternative.
...
- **Charts stay pie/donut.** `CategoryPieChart` is unchanged: dimension chip row, donut, legend with count and average. (An earlier version of this document specified ranked bar lists and a "breakdown card" — that is revoked; see `UPDATE_BRIEF.md` §9.)
