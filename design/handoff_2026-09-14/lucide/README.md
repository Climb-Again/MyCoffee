...
Fifteen files, one per icon in §11 of `UPDATE_BRIEF.md`. Source: [lucide.dev](https://lucide.dev), ISC licence. Open `index.html` to see the set.
...
| `list-filter.svg` | Filter (the outline funnel this replaces) |
| `settings.svg` | Nav bar — settings |
| `coffee.svg` | Tab — Coffees |
| `circle-plus.svg` | Tab — Add |
| `bar-chart-3.svg` | Tab — Insights |
| `book-open.svg` | Tab — Recipes |
| `shopping-bag.svg` | Tab — Shop |
