...
| 50 | ios-ux    | 6 | ready   | 46 | **Insights charts: per-label average rating + tap-to-filter** (Radu, 2026-08-14). **(a) DONE 2026-08-15 in `85d6911`** (shipped as part of the #52 Insights redesign) — every pie legend row now shows the slice's average rating (`Ethiopia · 12 · ★4.3`), threaded from `FacetCounts.Entry.averageRating` into `PieSlice`. **(b) STILL OPEN — this is what keeps the row `ready`:** Two parts to every Insights breakdown chart (`CategoryPieChart` and the per-dimension legends in `InsightsView`): (a) in each legend row show that label's **average rating** next to its count — e.g. `Ethiopia · 12 · ★4.2`. The facet entries already carry `averageRating` (`FacetCounts.Entry.averageRating`, same value the filter pills show), so `slices(for:)` just needs to thread it into `PieSlice`. (b) **Tapping a legend label** deep-links to the Coffees listing filtered to that value — e.g. tap "Ethiopia" in the Origin chart → Coffees tab, `CoffeeFilter` set to `originCountryIDs=[Ethiopia]`. Needs a cross-view action: set `CoffeeStore.filter` (+ `unknownDimensions` for the Unknown slice) and switch the `RootTabView` selection to Coffees. `CoffeeStore.filter` is shell-owned but already writable; the tab selection needs a shared `@Published`/binding the UX lane can set — **seam row, claim in `status/ios-shell.md` too** if a tab-selection surface has to be added. `Features/Insights/{InsightsView,InsightsCharts}.swift`, `Features/Root/RootTabView.swift`. |
