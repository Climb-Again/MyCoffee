...
| 29 | backend   | 6 | ready   | 26 | Harden the incremental path — of the three original parts, two are already done: `launchd` monthly Mac schedule is superseded by the CCR daily-extraction routine (already noted below); the `awaiting_text` deadline sweep has been live since #24 (`worker.js`'s `claimBatch` query already claims any photo whose `text_wait_until` has passed, and the daily routine triggers it once a day). **Lane corrected `data` → `backend` 2026-08-16** — the one remaining piece, `POST /api/admin/sync` (PLAN.md §6.7/§9), touches `routes/admin.js`, not any data-owned path, and its purpose beyond the existing `POST /api/admin/jobs`/`POST /api/admin/adjudicate` endpoints is unspecified in PLAN.md. Backend: either spec-and-build it, or decide the daily routine + built-in sweep already satisfy "harden the incremental path" and close this row. See `status/data.md` for the full investigation. |
...
| 59 | data      | 6 | claimed | — | **Root cause of sideways photos: the Mac exporter's `sips` step drops EXIF orientation** (Radu, 2026-08-15). **Code-complete 2026-08-16**: `ops/mycoffee_export.py`'s `convert_to_jpeg` now reads the source's EXIF orientation (`read_exif_orientation`, a `sips -g orientation` query) and bakes the matching `--rotate`/`--flip` flags into the same `sips` invocation that does the format/resize, via a new pure `sips_orientation_args` mapping (10 new table-driven tests, 31/31 `npm`-equivalent `python3 -m unittest` green). **Not yet on `main`** — this session's push is restricted to its own branch (`claude/peaceful-mccarthy-erypad`); stays `claimed` per `status/README.md`'s "done means on the shared branch" rule until an authorized session fast-forwards `main` (clean fast-forward — the branch started at `main`'s exact tip). **Not verified against a real `sips` binary** (no macOS runner in this sandbox) — the EXIF-orientation mapping is the universal standard one, but the exact `sips` flag spelling is unverified pending a real Mac run; flagged in `ops/README.md`'s gate checklist. Note: this only fixes photos ingested AFTER the fix lands; already-uploaded sideways photos still need #57's persisted rotate or a re-export from the Mac. See `status/data.md` for full detail. `ops/mycoffee_export.py`, `ops/test_mycoffee_export.py`. |
...
**🌱 2026-08-16 (data lane) — #59 code-complete on this session's own branch (not `main` yet); #29 corrected `data`→`backend`.**
`ops/mycoffee_export.py`'s `sips` conversion was dropping EXIF orientation
without rotating pixels first, serving photos sideways in the app. Fix bakes
the correction into the same `sips` call via a new pure
`sips_orientation_args` mapping + `read_exif_orientation` glue — 31/31
`ops/test_mycoffee_export.py` green (up from 29), 252/252 backend `npm test`
unaffected. **Stays `claimed`, not `done`** — this session's harness-assigned
branch (`claude/peaceful-mccarthy-erypad`) isn't `main`, same restriction
prior data-lane sessions (`rwi2ql`/`kix48i`/`6n4yw7`) hit; needs a
fast-forward by an authorized session (clean FF — branch started at `main`'s
exact tip). Separately, investigated `#29` (the only other `ready` data row)
and found two of its three parts already done (`launchd` superseded by the
daily routine; the `awaiting_text` sweep has been live in `worker.js` since
#24) and the third (`POST /api/admin/sync`) touches `routes/admin.js`, not
any data-owned path — corrected the lane tag to `backend`, same precedent as
#14's original mistagging. Full writeup in `status/data.md`.
