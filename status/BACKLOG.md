...
| 48 | data      | 4 | claimed | — | **Roaster country should trust the caption over the vocab guess.** Code done + tested 2026-08-13 (both parts): `backend/migrations/015_uncommon_country_fix.sql` corrects `Uncommon`'s `country_id` (UK→Netherlands, matching the one real coffee's own caption) + seeds the missing `'Olanda'` alias; `deterministic.js`'s new `extractRoasterCountryField()` proposes `roaster_country_id` from an explicit roastery-labelled caption line so it can outvote the vocab-derived default in general, not just for this one case. Verified end-to-end against a real local Postgres 16, incl. a live `runWorker` run proving a fabricated caption's country beats a *correct* vocab-derived one. **Left at `claimed`, not `done`** — code is only on this session's own branch (`claude/peaceful-mccarthy-71uw7l`), not yet on `main`; needs an authorized session to merge (same gap `status/README.md` describes for `rwi2ql`/`kix48i`). Full detail in `status/data.md`. |
...
| 39 | data      | 4 | claimed | — | **Accept-by-default needs field sanity envelopes** (PLAN.md §11 addendum). Code done + tested 2026-08-13: `parseAltitude`/`parseWeight`/`parseRating` in `src/lib/normalize.js` each got a hard plausibility envelope (`return null` when a parse can't be the real quantity — e.g. `max<200`/`min>4000` for altitude, exactly Radu's own "1–5 m" example) on top of the existing soft `needsReview` band. `npm test` 236/236 green. **Left at `claimed`, not `done`** — only on this session's own branch (`claude/peaceful-mccarthy-71uw7l`), not yet on `main`; needs an authorized session to merge. Full detail in `status/data.md`. |
...
**🌍 2026-08-13 (data lane) — #39 and #48 code-complete, tested, but stranded on this session's own branch.** Both `ready` data rows this cycle, both finished and verified end-to-end against a real local Postgres 16 (`npm test` 236/236). Left at `claimed` in the table above, not `done` — this session's harness restricts `git push` to `claude/peaceful-mccarthy-71uw7l` only, so `main` (`4d0a771` as of this session) hasn't moved. That branch is a clean fast-forward from `main`; whoever can push `main` directly should merge it, then flip both rows to `done`. Full writeup (including the live `runWorker` proof for #48) in `status/data.md`.
