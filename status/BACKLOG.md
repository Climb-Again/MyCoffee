...
| 58 | ios-ux    | 6 | done    | — | **Move the listing search box to the bottom (iOS 26 style)** (Radu, 2026-08-15) — DONE. Root cause of why it stayed docked at the top: `RootTabView` built its `TabView` with the legacy `.tabItem`/`.tag()` pair, which iOS 26 never gives the new bottom-tab-bar-integrated search treatment — only the value-based `Tab(_:systemImage:value:)` builder (iOS 18+) does. `RootTabView.swift` now branches on `#available(iOS 18, *)`: the modern `Tab(...)` builders (with `.badge` moved onto the Review `Tab` itself) on iOS 18+, the original `.tabItem`/`.tag()` form kept verbatim as the iOS 17 fallback (deployment target stays 17.0 per `CLAUDE.md` §9). `CoffeesListView.swift`'s own `.searchable(text: $store.filter.query, prompt:…)` call is untouched — same binding, same prompt, same placement API — the row's own "placement only" framing holds; only the tab declaration it's nested under changed, which is what iOS 26 keys off to relocate the field near the bottom tab bar. Doesn't collide with the listing's sticky section headers (those are `List` section headers, unrelated to the tab-bar-anchored search surface). **Flagging the one thing I can't verify without a device/Xcode**: that iOS 26 really does dock a `Tab`-builder tab's `.searchable` field at the bottom automatically, with no further opt-in modifier — this is the mechanism the row's own text points at (rather than a dedicated `Tab(role: .search)`, which would add a 4th tab-bar affordance and break the "three tabs" decision) but I have no simulator to screenshot-verify the rendered placement against. If the compile lane's build renders it back at the top on a real device, the next thing to check is whether `Tab`'s `role:` parameter or an explicit placement modifier is additionally required — not a syntax problem, a behavior one. `ios/MyCoffee/Sources/Features/{Root/RootTabView,Coffees/CoffeesListView}.swift`. |
