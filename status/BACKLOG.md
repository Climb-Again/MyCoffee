...
| 39 | data      | 4 | ready   | — | **Accept-by-default needs field sanity envelopes** (PLAN.md §11 addendum). Radu: "accept all guesses, but altitude 1–5 m doesn't make sense." #35 applies the top pick regardless of confidence, so a mis-parsed `1–5 m` altitude now shows. Give `parseAltitude` a hard reject (`max<200` or `min>4000` → null → field absent) beyond the soft `needsReview` band; same "obviously-not-this-quantity → null" guard for `parseWeight`/`parseRating`. $0 to validate via re-adjudication. `src/lib/normalize.js` — **note:** this row used to be malformed, merged as trailing text onto #43's table row (a markdown corruption, not a content change) rather than its own row; split back out here by the #43/#44 backend session so Data's own row-scan can find it. |
