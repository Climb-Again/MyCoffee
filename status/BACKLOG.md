...
| 61 | backend   | 6 | ready   | — | **Cut the Vertex extraction bill (~5–6¢/photo → target ~1–1.5¢)** (Radu, 2026-08-15: "take the vertex bill down"). Per-photo the `full` voter set runs 4 LLM calls (`src/lib/agents.js`), and the cost is dominated by **two `gemini-2.5-pro` voters left with thinking ON** — `runExtractA` (pro, no thinkingBudget) and `runReconciler` (pro, no thinkingBudget). Pro output is $10/M tok vs flash $2.5/M, and thinking tokens bill as output on a 2.5 thinking model (`estimateCostUsd` sums `candidatesTokenCount + thoughtsTokenCount`). `extract_b` + `critic` are already flash w/ `thinkingBudget:0` (cheap); `rules` is deterministic (free). Levers, biggest first: (1) **move `extract_a` and `reconciler` off pro to `gemini-2.5-flash`** (flash allows `thinkingBudget:0`, pro can't — see the vertex.js guard) → ~4× cheaper + no thinking, est. 70–80% off; (2) if pro is kept for the reconciler's judgement, **set a tight `thinkingBudget` (512–1024)** instead of default; (3) **drop a voter** (the critic, or one extractor) for proportional savings. **Validate before committing the corpus:** run a 5-photo batch with the cheaper config and diff extracted fields vs current pro output for accuracy regressions (re-*adjudication* is $0 and reuses stored candidates; only re-*extraction* spends). Note ongoing bill is near-zero once backfill completes — this mainly caps the tail of the backfill and any future prompt-version re-extraction. Coordinate with data lane on prompts (`src/lib/prompts.js`) if prompts need tightening for flash. `src/lib/agents.js` (voter models/thinking), `src/lib/worker.js` (voter set), maybe `src/config.js` (make the extractor model an env var). |
