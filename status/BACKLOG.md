...
| 66 | ios-ux    | 6 | done    | — | **VERIFIED ALREADY CORRECT 2026-08-16 — no code change needed.** Traced the full path: `ReviewQueueEngine.accept`/`.markNotPresent` call `onAccept?`/`onDismiss?` synchronously in the same call that mutates the local queue (no batching, no "done" step gate); `ReviewQueueView`'s hooks call `store.resolveReview`/`.dismissReview` immediately; `CoffeeStore.resolveReview`/`.dismissReview` fire a `Task` immediately (not deferred); `MutationOutbox.enqueueReviewResolve`/`.enqueueReviewDismiss` call `persist()` synchronously right after enqueueing, before any network round-trip. The row's own verification ask ("action 2 of N items, force-quit, relaunch → those 2 are still resolved server-side") is satisfied by this chain as it stands — each action is durably queued to disk the instant it's actioned, then flushed opportunistically. **Review must save each item as it's actioned — not only at the end** (Radu, 2026-08-16: "Review does not save one by one — I don't want to have to finish all items to save. Every step should be saved."). On the Review screen, accepting/dismissing an item should persist immediately (per-item), so quitting partway keeps everything already actioned. Investigate the Review flow (`Features/Review/**`, `CoffeeStore.resolveReview`/`dismissReview` → `CoffeeRepository`): those already go through the offline outbox, so the likely cause is the **UI batching** the actions into local state and only flushing/committing on a final "done"/submit, or advancing the card without dispatching the mutation until the queue is finished. Fix: dispatch the resolve/dismiss mutation the moment the user actions each card (the outbox already makes it durable), and update the badge/queue optimistically. Verify: action 2 of N items, force-quit, relaunch → those 2 are still resolved server-side. `Features/Review/**`, `Store/CoffeeStore.swift` seam if needed. |
