...
| 57 | ios-ux    | 6 | ready   | 59 | **Persisted "rotate photo" — the correction must SAVE** (Radu, 2026-08-15: "Some photos are rotated even though were ok on Photos on Mac. I need a permanent fix. So rotate should save."). A rotate control in the photo viewer whose result **persists across launches and devices** (scope question resolved — persistence is required, not view-only; this supersedes the earlier view-only-default draft of this row). Cross-lane, and pairs with the root-cause fix #59 (which stops NEW photos coming in sideways; #57 corrects the ones already uploaded, whose stored source `ocr` asset is itself baked sideways so a server re-derive alone can't fix them). Recommended mechanism (cheap, no re-encode, syncs everywhere): **store a per-photo `rotation_quarter_turns` (0–3) on the photo/coffee**, carry it in the snapshot (detail at least, ideally the compact row so the listing thumb is right too), and have iOS apply it on display; the viewer's rotate button bumps it via an `/edit`-style write. (Alternative: server re-encodes `display`/`thumb` from a corrected source — heavier, and can't help where the retained `ocr` source is already wrong.) **Lanes:** backend (column + write endpoint + snapshot field), ios-shell (model field + API + apply on the `CoffeeImage`/detail image), ios-ux (rotate button in `CoffeeDetailView` + the #55 full-screen viewer). Seam row — claim in `status/ios-shell.md` and `status/backend.md` too. `Features/Coffees/CoffeeDetailView.swift`, the #55 viewer, `Store/`+`API/` (shell), `routes/coffees.js`+a migration (backend). |
| 58 | ios-ux    | 6 | claimed | — | **Move the listing search box to the bottom (iOS 26 style)** (Radu, 2026-08-15) — code done, **not yet on `ios-staging`**. `CoffeesListView`'s `.searchable` now requests `SearchFieldPlacement.tabBar` on iOS 26+ (docks near the tab bar, thumb-reach), gated `if #available(iOS 26.0, *)` with a fallback to the original top placement on 17–25 — same binding (`$store.filter.query`) and prompt either way, placement only, no `RootTabView` change needed. Landed on `claude/hopeful-johnson-bdpy3r` (this session's own designated branch, not `ios-staging` directly — the branch carries `ios-staging`'s full history merged in first, so it's a strict superset ready to integrate, not a fork). See `status/ios-ux.md` for detail; `.tabBar` is the least-precedented API in this codebase (no local Xcode to confirm the exact case name) — flag the compile lane to `Features/Coffees/CoffeesListView.swift`'s `searchableList` first if the next check goes red, once merged. **A session that can push to `ios-staging`: merge `claude/hopeful-johnson-bdpy3r` in and flip this row to `done`.** `Features/Coffees/CoffeesListView.swift`. |
