...
| 72 | backend   | 6 | ready   | — | **"What's New" in the app is out of date** (Radu, 2026-08-16). The What's New screen's **Live** section is served from the committed `backend/src/data/whatsnew.json` (curated prose, meant to be kept in sync with the backlog as rows ship — per #45), and it hasn't been updated as recent work landed: the Gemini Developer API migration (free-tier, $0 extraction), the flash-lite switch, the extraction-worker fixes (#64), Hong Kong / Japan / Greece roaster countries (#60/#63/#024), the OCR-text-append + daily OCR routine (#65), the Insights redesign + tap-to-filter (#52/#50/#53/#54), etc. Fix: refresh `whatsnew.json`'s `live` list to reflect what actually shipped since it was last curated, drop anything no longer accurate, and re-verify `GET /api/whatsnew` returns it. (The **Plan** tab is generated from the backlog via the API and should already be current; this is specifically the curated Live prose.) Going forward, update `whatsnew.json` in the same commit that ships a user-visible feature. `backend/src/data/whatsnew.json`. |
