...
| 40 | backend   | 5 | ready   | 36 | **Generic per-field edit endpoint** (PLAN.md §12). `POST /api/coffees/:publicId/edit` `{field, value}` (+ batch) — canonicalize + write a `locked`/`decided_by='human'` resolution + apply + close open review items, for ANY field on ANY coffee (not just flagged ones). Add the missing `roaster_country_id` canonicalize + column-update case. Refactor review.js's resolve body into a shared `resolveField()` helper both routes call. Countries stay closed (422 on unknown). `routes/coffees.js`, `routes/review.js`, `src/lib/adjudicate.js`, `worker.js` |
| 41 | ios-shell | 5 | blocked | 40 | **Edit API surface** (PLAN.md §12). `APIClient.editCoffeeField(publicId:field:value:)`; `CoffeeRepository.editField` via the mutation outbox + detail re-fetch; `CoffeeStore.editField`. Dropdown vocab already on-client (`index.vocabulary` + `Profile` enum) — no new read endpoint. Seam row: also claim in `status/ios-ux.md` |
