...
| 82 | ios-ux    | 6 | blocked | 81 | **Coffee page: show flavour notes as their own section — SHIP THIS INDEPENDENTLY, don't wait for the full redesign** (Radu, 2026-08-25/26 — "show flavour notes separately, usually 3 to 5 words"; explicitly queued for the lanes). Data is 100% populated (#79/#80 done — every coffee has `flavorNotes`), so the only thing between Radu and seeing them is this UI. On `CoffeeDetailView`, render `coffee.flavorNotes` (a comma-separated string; split on commas) as a distinct labelled "Flavour profile" section — short chips, ~3–5 items, shown only when non-empty. **Style per the design handoff** (`design/coffees_redesign/README.md` §"FLAVOUR PROFILE": 10pt/0.12em label, chips 11pt semibold on `#eef6ff` fill / `#00337f` text, radius 999) so it's already correct when the #88 detail redesign lands — #88 then owns the surrounding page, this row owns the flavour section itself. Needs #81's `Coffee.flavorNotes` wire. `Features/Coffees/CoffeeDetailView.swift` (+ a small `DesignSystem/` chip if none fits). Renumbered from #78. On `CoffeeDetailView`, render `coffee.flavorNotes` as a distinct labelled section — short chips or a one-line "Flavour notes: …" — clearly separate from the process row and the Full-text/prose blocks, shown only when present. ~3–5 words, so a compact chip row (reuse `WrapLayout`/`FilterPill` styling) reads best. Needs #77. `Features/Coffees/CoffeeDetailView.swift` (+ a small `DesignSystem/` chip if none fits). |
