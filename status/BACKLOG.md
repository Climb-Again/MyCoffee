...
| 63 | backend   | 6 | ready   | — | **Add Japan to roaster countries** (Radu, 2026-08-16). New forward-only migration adding Japan to `countries` as a **roaster** country (not origin): `INSERT INTO countries (name, iso2, is_origin, is_roaster, kind) VALUES ('Japan','JP',false,true,'country') ON CONFLICT DO NOTHING;` — same shape as #60 (Hong Kong). **Batch #60 + #63 into one migration** (`0NN_add_hong_kong_japan_roaster_countries.sql`) since they're identical one-liners. Deploy per CLAUDE.md §12 (check `GET /api/admin/jobs` shows no `running` job first) and verify both appear in the roaster-country picker. `backend/migrations/`. |

| 64 | backend   | 6 | ready   | — | **Worker infinite-loops on a photo that always fails → holds the advisory lock forever (stuck lock)** (found live 2026-08-16). `runWorker` loops `while (photosDone < limit)`; `claimBatch` selects `state <> 'processed' AND (text_received OR (awaiting_text AND text_wait_until<=now())) AND extraction_leased_until IS NULL`, and on a per-photo failure the catch calls `releaseLease(photo.id)` (nulls the lease) — so a photo that *always* fails is immediately re-claimable next round. `photosDone` never increments (only successes count), `limit` is never reached, and `claimBatch` never returns empty, so the job never terminates on its own: the worker spins forever holding `pg_advisory_lock`, blocking every other job (`another worker holds the extraction advisory lock`) and burning CPU. This is what caused today's repeated stuck locks; the daily extraction routine (`trig_01JWhQADZK8RqfP8r9ugXen1`) was **disabled** as mitigation so it doesn't hang the worker nightly — re-enable once fixed. The trigger today: image-only photos (`awaiting_text`, text never arrived, `text_wait_until` now elapsed) get claimed but have no text to parse in text-only mode → Gemini `400 INVALID_ARGUMENT` every time → loop. Fix options: (a) track a per-photo failure count and stop re-claiming after N fails (mark a `failed`/`error` state so `claimBatch` excludes them); and/or (b) in text-only mode (`includeImages:false`) don't claim `awaiting_text` photos at all — they have no text and never will without image OCR; give them a terminal state like `no_text`. Either makes the job reach `no_work` and release the lock. `src/lib/worker.js` (`runWorker`/`claimBatch`), `test/worker.test.js`. |
| 65 | data      | — | human   | — | **The last handful of photos have an image but no caption/description text — need an image-OCR run, or leave them** (Radu decision, 2026-08-16). The text backfill is COMPLETE (172 coffees = every photo that carries caption/description text). The few photos still unprocessed are image-only (state `awaiting_text`, text never arrived from the Mac export). In the standing **text-only** mode (`includeImages:false`) they can't be parsed — there's no text, so "parse directly" can't recover them either; the bag details live only in the image pixels. To extract them you'd run extraction once with `includeImages:true` (image OCR — the mode Radu turned off), scoped to just those photos. Options: (1) leave them unextracted (backfill is effectively done); (2) one-off `includeImages:true` run for the stragglers (small Gemini spend / flash-lite free-tier calls). **Blocked on #64** — don't trigger any run until the infinite-loop is fixed, or it'll hang the worker again. |
