...
| 37 | ios-ux    | 5 | ready   | 35, 36 | **"Needs review" reflects only actionable items** (PLAN.md §11). The coffee-page Review button opens an empty sheet when a coffee's open items are all non-reviewable fields. Gate the affordance on a per-coffee reviewable count (backend can expose `openReviewCount` on detail) / keep the "All set" empty state. Mostly resolved now that #35 landed (the live queue collapsed) — this is now the client belt-and-braces only. |
