...
| 60 | backend   | 6 | ready   | — | **Add Hong Kong to roaster countries** (Radu, 2026-08-15). New forward-only migration adding Hong Kong to `countries` as a **roaster** country (not origin): `INSERT INTO countries (name, iso2, is_origin, is_roaster, kind) VALUES ('Hong Kong','HK',false,true,'country') ON CONFLICT DO NOTHING;` — same shape as `017_add_switzerland_roaster_country.sql` / `018_add_cameroon_origin_country.sql`. Pick the next migration number. Deploy (push `backend/**` to `main` — check `GET /api/admin/jobs` shows no `running` job first, per CLAUDE.md §12) and verify Hong Kong appears in the roaster-country picker. `backend/migrations/0NN_add_hong_kong_roaster_country.sql`. |
