...
| 44 | backend   | 4 | ready   | — | **Auto-create farms during adjudication** (accept-by-default, PLAN.md §11). Farm is 0/21 in the app: farm names ARE extracted (they show as review candidates) but auto-adjudication resolves `origin_farm_id` via vocab only — 0 farms seeded — so a confident farm never applies; #36's get-or-create fires only on a human accept. Apply the same get-or-create to a confident `accepted` `origin_farm_id` in the worker/adjudication path so extracted farms populate without manual review (open-ended vocab; the edit sheet #42 corrects mistakes). Consider the same for a confident new roaster. $0 to validate via `POST /api/admin/adjudicate`. `src/lib/worker.js` / `resolveField.js` |
| 43 | backend   | 5 | ready   | — | **Optimize served photos so caching stays under budget** (CLAUDE.md 50 MB rule). Radu: "instead of not caching, optimize photos." The 30 MB on-device cache + launch eviction (shipped, `ImageStore`) is the backstop; the real lever is smaller `display` bytes so many photos fit without evicting. In `routes/photos.js` `DERIVATIVES`, shrink `display` (1290 px/q82 → ~1080 px/q72, or switch to WebP `image/webp` — iOS AsyncImage supports it; would touch `media.js` VARIANTS/extension + the `/media` content-type). Add an admin **re-derive** pass so the existing ~21 photos shrink too (new uploads get the new spec automatically). Leave `ocr` (2048) as-is — extraction-only, never cached on device. Check `GET /api/admin/jobs` (no `running`) before pushing `backend/**`. Measure a display's bytes before/after |
